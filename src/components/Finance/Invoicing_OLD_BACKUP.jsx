...
import {
  ...
  Stat, StatLabel, StatNumber, StatHelpText, Menu, MenuButton, MenuList, MenuItem, MenuDivider
} from "@chakra-ui/react";
import { FiDownload, FiEye, FiPlus, FiEdit2, FiTrash2, FiPrinter, FiUpload, FiInfo, FiChevronDown, FiMoreHorizontal, FiTrendingUp } from "react-icons/fi";
...
const FinanceInvoicing = () => {
  ...
  // ===== NOUVEAUX HANDLERS POUR LE WIZARD UNIFIÉ =====
    ...
    try {
      ...
      if (documentData.mode === 'generate' && documentData.lines?.length > 0) {
        try {
          ...
          const linesPayload = documentData.lines.map((line, idx) => ({
            [idKey]: result.id,
      ...
      if (documentData.mode === 'generate' && result?.id) {
        try {
