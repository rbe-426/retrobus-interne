...
export default function IneoFreeTracking() {
  ...
  const createSession = async () => {
    const stopByStop = window.confirm('Activer le mode Stop by Stop ?\n\nLe conducteur pourra marquer chaque arrêt à l’arrêt du véhicule. Les distances et temps entre les arrêts seront enregistrés.');
    try {
      setStarting(true);
      const data = await ineoAPI.createFreeTrackingSession({ stopByStop });
      setCreatedSession(data.session);
      setSelectedSessionId(data.session.id);
      setSessions((current) => [data.session, ...current]);
      toast({ status: 'success', title: 'Code course libre généré', description: `${data.session.serviceReference} · ${data.session.courseCode}${stopByStop ? ' · Stop by Stop activé' : ''}` });
    } catch (error) {
  ...
  if (manager) return <Container maxW="5xl" py={8}><VStack align="stretch" spacing={5}>
    ...
    <Tabs variant="enclosed" colorScheme="blue"><TabList><Tab>Codes course libres</Tab><Tab>Suivi véhicule</Tab></TabList><TabPanels><TabPanel px={0} pt={5}><VStack align="stretch" spacing={5}>
      <Box border="1px solid" borderColor="#c6d0d8" bg="#f8fafb" p={5}><HStack justify="space-between" flexWrap="wrap" gap={4}><Box><Text fontWeight="700">Nouveau code course libre</Text><Text fontSize="sm" color="gray.600">Service fixe : RBE-999-999 · le mode Stop by Stop est proposé avant génération.</Text></Box><Button colorScheme="blue" leftIcon={<FiPlay />} isLoading={starting} onClick={createSession}>Générer le code course</Button></HStack></Box>
      ...
      <Box border="1px solid" borderColor="#c6d0d8" overflowY="auto" maxH="340px"><Text px={4} py={3} fontWeight="700" bg="#e9eff3">Codes libres générés</Text>{sessions.length ? sessions.map((item) => <Button key={item.id} w="full" h="auto" px={4} py={3} borderTop="1px solid" borderRadius="0" borderColor="#e2e8f0" justifyContent="space-between" variant="ghost" onClick={() => setSelectedSessionId(item.id)}><HStack><Text fontFamily="monospace">{item.serviceReference} · {item.courseCode}</Text>{item.stopByStop && <Badge colorScheme="purple">STOP BY STOP</Badge>}</HStack><Badge colorScheme={item.status === 'ACTIVE' ? 'green' : item.status === 'PENDING' ? 'orange' : 'gray'}>{item.status}</Badge></Button>) : <Text px={4} py={5} color="gray.500">Aucun code course libre généré.</Text>}</Box>
    </VStack></TabPanel><TabPanel px={0} pt={5}>{selectedManagedSession ? <VStack align="stretch" spacing={4}><Box border="1px solid" borderColor="#c6d0d8" bg="#f8fafb" p={5}><HStack justify="space-between" flexWrap="wrap" gap={4}><Box><Text fontFamily="monospace" fontWeight="700">{selectedManagedSession.serviceReference} · {selectedManagedSession.courseCode}</Text><Text mt={1} fontSize="sm" color="gray.600">{selectedManagedSession.vehicleParc || 'Véhicule en attente d’affectation'} · {selectedManagedSession.driverName || selectedManagedSession.driverIdentifier || 'Conducteur en attente d’affectation'}</Text></Box><Badge colorScheme={selectedManagedSession.status === 'ACTIVE' ? 'green' : selectedManagedSession.status === 'PENDING' ? 'orange' : 'gray'}>{selectedManagedSession.status}</Badge></HStack><Grid mt={5} templateColumns={{ base: '1fr 1fr', md: 'repeat(3, 1fr)' }} gap={4}><Stat><StatLabel>Vitesse</StatLabel><StatNumber>{formatSpeed(selectedManagedSession.lastSpeedKmh)}</StatNumber></Stat><Stat><StatLabel>Vitesse max.</StatLabel><StatNumber>{formatSpeed(selectedManagedSession.maxSpeedKmh)}</StatNumber></Stat><Stat><StatLabel>Distance</StatLabel><StatNumber>{formatDistance(selectedManagedSession.distanceMeters)}</StatNumber></Stat></Grid></Box><FreeTrackingMap session={selectedManagedSession} points={selectedManagedSession.positions || []} />{selectedManagedSession.stopByStop && <Box border="1px solid" borderColor="#c6d0d8"><Text px={4} py={3} fontWeight="700" bg="#e9eff3">Arrêts marqués</Text>{selectedManagedSession.stops?.length ? selectedManagedSession.stops.map((stop) => <HStack key={stop.id} px={4} py={3} borderTop="1px solid" borderColor="#e2e8f0" justify="space-between"><Text>Arrêt {stop.sequence} · {new Date(stop.recordedAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</Text><Text fontSize="sm" color="gray.600">{formatDistance(stop.distanceMeters)} · {Math.round(stop.durationSeconds / 60)} min</Text></HStack>) : <Text px={4} py={3} color="gray.500">Aucun arrêt marqué.</Text>}</Box>}</VStack> : <Box border="1px solid" borderColor="#c6d0d8" p={5} color="gray.600">Sélectionnez un code course libre dans le premier onglet pour suivre son véhicule.</Box>}</TabPanel></TabPanels></Tabs>
  </VStack></Container>;
