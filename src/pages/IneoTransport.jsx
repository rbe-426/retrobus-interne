...
import { FiChevronDown, FiChevronRight, FiCopy, FiEdit2, FiMapPin, FiPlay, FiPlus, FiRefreshCw, FiSave, FiSearch, FiSmartphone, FiStopCircle, FiTrash2 } from 'react-icons/fi';
...
const findClosestSegmentIndex = (position, stops) => {
  ...
  stops.slice(0, -1).forEach((start, segmentIndex) => {
    ...
    if (distance < closest.distance) closest = { distance, segmentIndex };
  });
  return closest.segmentIndex;
};
...
// Delay is based on the reference stop actually being passed, not the service's final arrival.
const getDelayMinutes = (tracker, mission, routes) => {
  if (mission?.status !== 'ACTIVE' || tracker?.lastLatitude == null || tracker?.lastLongitude == null) return null;
  const route = routes.find((item) => String(item.courseReference || '').toUpperCase() === String(mission.courseReference || '').toUpperCase());
  const stops = (route?.stops || []).filter((stop) => Number.isFinite(stop.lat) && Number.isFinite(stop.lng));
  if (stops.length < 2) return null;
  const segmentIndex = findClosestSegmentIndex({ lat: tracker.lastLatitude, lng: tracker.lastLongitude }, stops);
  const referenceStop = [stops[segmentIndex], stops[segmentIndex - 1], stops[segmentIndex + 1]].find((stop) => stop?.scheduledTime);
  const referenceTime = referenceStop ? parseStopTime(referenceStop.scheduledTime, new Date()) : null;
  if (!referenceTime) return null;
  const deltaMinutes = Math.round((Date.now() - referenceTime.getTime()) / 60000);
  return deltaMinutes >= -2 && deltaMinutes <= 5 ? 0 : deltaMinutes;
};
...
export default function IneoTransport({ vehicles, initialTab = 'positions', showTabSelector = true, routeWorkspace = false }) {
  ...
  const prepareDailyCourse = (route) => {
    setSelectedRoute({ ...route, id: undefined, courseReference: '' });
    setCourseModalOpen(true);
  };
  const removeRoute = async (route) => {
    if (!window.confirm(`Supprimer le trajet « ${route.routeName} » et son code course ${route.courseReference} ?`)) return;
    try {
      setSaving(true);
      await ineoAPI.removeRouteByReference(route.courseReference);
      setRoutes((current) => current.filter((item) => item.courseReference !== route.courseReference));
      toast({ status: 'success', title: 'Trajet supprimé' });
    } catch (error) {
      toast({ status: 'error', title: 'Suppression impossible', description: error.message, duration: 9000, isClosable: true, position: 'top' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return <VStack align="stretch" spacing={4}>
    <Box borderBottom="1px solid" borderColor="#b7c4cd" pb={3}><HStack justify="space-between" flexWrap="wrap" gap={3}><Box><Text fontSize="20px" fontWeight="700" color="#17364d">{routeWorkspace ? 'Trajets et codes course' : 'Carte de transport'}</Text><Text color="#60727e">{routeWorkspace ? 'Enregistrez un trajet une fois, puis préparez son code course pour la journée d’exploitation.' : 'Suivi des appareils Urbex, courses, lignes et itinéraires.'}</Text></Box><Button size="sm" borderRadius="2px" leftIcon={<FiRefreshCw />} isLoading={loading} onClick={load}>Actualiser</Button></HStack></Box>
    {showTabSelector && <HStack spacing={2} borderBottom="1px solid" borderColor="#c6d0d8" pb={3}><SubTab active={tab === 'positions'} onClick={() => setTab('positions')}>Position VH</SubTab><SubTab active={tab === 'routes'} onClick={() => setTab('routes')}>Codes service et course</SubTab></HStack>}
