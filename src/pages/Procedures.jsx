...
export default function Procedures() {
  ...
  return <PageLayout title={selectedCategory?.title || 'Procédures'} subtitle={selectedCategory ? 'Documents associés à cette catégorie' : 'Référentiel opérationnel RétroBus Essonne'} headerVariant="card" bgGradient="linear(to-r, purple.600, purple.800)" titleSize="lg"><VStack spacing={6} align="stretch">
    {selectedCategory ? <><HStack justify="space-between"><Button leftIcon={<FiArrowLeft />} variant="ghost" onClick={() => navigate('/dashboard/procedures')}>Toutes les catégories</Button>{canPublish && <Button colorScheme="blue" leftIcon={<FiUpload />} onClick={onOpen}>Importer un PDF</Button>}</HStack>
      {loading ? <Text color="gray.500">Chargement des procédures...</Text> : documents.length ? <SimpleGrid columns={{ base: 2, sm: 3, md: 4, lg: 5 }} spacing={4}>{documents.map((document) => <Card key={document.id} overflow="hidden" borderWidth="1px" _hover={{ shadow: 'md', transform: 'translateY(-2px)' }} transition="all .2s"><Box as="a" href={`${API_BASE}${document.filePath}`} target="_blank" rel="noreferrer" display="block"><PdfThumbnail document={document} /></Box><CardBody p={3}><Heading size="xs" noOfLines={2}>{document.title}</Heading>{document.description && <Text mt={1} fontSize="xs" color="gray.600" noOfLines={3}>{document.description}</Text>}<HStack mt={3} justify="space-between"><Text fontSize="xs" color="gray.500" noOfLines={1}>{document.fileName}</Text>{canPublish && <IconButton size="xs" colorScheme="red" variant="ghost" aria-label={`Supprimer ${document.title}`} icon={<FiTrash2 />} onClick={() => deleteDocument(document)} />}</HStack></CardBody></Card>)}</SimpleGrid> : <Box py={12} textAlign="center" border="1px dashed" borderColor="gray.300"><FiBook size={28} /><Text mt={3} color="gray.600">Aucune procédure publiée dans cette catégorie.</Text></Box>}<ProcedureUploadModal category={selectedCategory} isOpen={isOpen} onClose={onClose} onPublished={(document) => setDocuments((current) => [document, ...current])} /></>
      : <><Box><Heading size="md" mb={2}>Référentiels par activité</Heading><Text color="gray.600">Choisissez la catégorie correspondant à votre opération.</Text></Box><SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>{procedures.map((procedure) => <Card key={procedure.id} cursor="pointer" borderWidth="1px" borderColor="gray.200" borderTopWidth="4px" borderTopColor={`${procedure.color}.500`} onClick={() => navigate(`/dashboard/procedures/${procedure.id}`)} _hover={{ shadow: 'md', transform: 'translateY(-2px)' }} transition="all .2s"><CardHeader pb={2}><HStack><Box color={`${procedure.color}.500`}><procedure.icon size={22} /></Box><Heading size="sm">{procedure.title}</Heading></HStack></CardHeader><CardBody pt={2}><Text fontSize="sm" color="gray.600">Consulter les documents de la catégorie</Text></CardBody></Card>)}</SimpleGrid></>}
